...
describe('Integration User/ ReadOne', () => {
  /**
   *
   * * Integration
   *
  ...
  it("it should return {error: 'not_found'} if it was deleted", async () => {
    ...
    const { error } = await UserService.getOne(anotherInstance._id);
  ...
  it("it should return {error: 'not_found'} if it was not found", async () => {
    ...
    const { error } = await UserService.getOne(new ObjectId());
    expect(error).to.not.be.undefined;
    expect(error.statusCode).to.equal(404);
  });
  ...
  it('it should return {data: true} if it passes all validations', async () => {
    ...
    const { data } = await UserService.getOne(instance._id);
